...
<table class="table">
	...
	<tr>
        ...
        <div class="input-prepend">
		<span class="add-on">學分</span>
            <input type="text" class="span1" name="credit" id="creidt" value="${cs.credit}" autocomplete="off" />
        </div>
        <div class="input-prepend">
		<span class="add-on">時數</span>
            <input type="text" class="span1" name="thour" id="thour" value="${cs.thour}" autocomplete="off" />
        </div>
        <div class="input-prepend input-append">
		<span class="add-on">上限</span>
            <input type="text" class="span1" name="Select_Limit" id="Select_Limit" value="${cs.Select_Limit}" autocomplete="off" />
	...
	<tr>
		<td>		
			<div class="input-prepend">
				...
				<input type="text" class="span3 techid" name="techid" id="techid" value="<c:if test="${!empty cs.techid}">${cs.techid}, ${cs.cname}</c:if>" onClick="this.value='';" autocomplete="off" data-provide="typeahead"/>
				
			</div>			
			<div class="input-prepend">
				<span class="add-on">科目名稱</span>
				<input type="text" class="span3 cscode" name="cscode" id="cscode" value="${cs.cscode},${cs.chi_name}" onClick="this.value='';" autocomplete="off" data-provide="typeahead"/>
			</div>
		</td>
	</tr>
	<tr>
		<td>		
			<div class="btn-group">			
			<button class="btn demo" onClick="javascript: return(confirm('確定刪除?')); void('')" name="method:delete">刪除課程</button>
		    <button class="btn btn-danger demo" name="method:saveDtime">儲存課程基本資料</button>			
		    </div>		    
		    <button name="method:back" class="btn">返回查詢列表</button>
		    <div class="btn-group">
