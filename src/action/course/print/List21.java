 ...
 *
 */
public class List21 extends BasePrintXmlAction{
	...
	public void print(HttpServletResponse response, List<Map>dtimeList, String term) throws IOException{
		...
		out.println("<td>修課人數</td>");
		out.println("<td>遠距</td>");		
		out.println("</tr>");
		
		
		StringBuilder sql=new StringBuilder("SELECT d.elearning, d.Oid, d.credit,cc.name as SchoolName,d.cscode,cs.chi_name, "
		...
		for(int i=0; i<cs.size(); i++) {
			...
			cs.get(i).put("stds", df.sqlGet("SELECT cs.name as SchoolName, st.student_name, st.student_no, c.Grade, "
					+ "c.SeqNo, cd.name as deptName FROM Dtime d, Seld se, stmd st, Class c, CODE_DEPT cd, CODE_SCHOOL cs "
					+ "WHERE cs.id=c.SchoolNo AND se.csdepart_class=c.ClassNo AND cd.id=c.DeptNo AND "
					+ "d.Oid=se.Dtime_oid AND c.ClassNo=d.depart_class  AND se.student_no=st.student_no AND "
		...
		for(int i=0; i<cs.size(); i++) {
			...
			try {
				...
				out.println("<td>"+stds.size()+"</td>");
				if(cs.get(i).get("elearning").toString().equals("0")) {
					out.println("<td></td>");
				}else {
					out.println("<td>遠距</td>");
				}
